<template lang="pug">
div
  span(v-if="show && dirty && field.required === false").
    Campo obrigatório
  span(v-if="show && dirty && field.maxLength === false").
    Campo deve ter menos de {{ field.$params.maxLength.max }} caracteres
  span(v-if="show && dirty && field.minLength === false").
    Campo deve ao menos {{ field.$params.minLength.min }} caracteres
  span(v-if="show && dirty && field.email === false").
    Este não é um e-mail válido
  span(v-if="show && dirty && field.sameAs === false")
